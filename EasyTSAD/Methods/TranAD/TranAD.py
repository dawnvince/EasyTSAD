...
class TranAD(BaseMethod):
    def __init__(self, params: dict) -> None:
        ...
        self.model = TranADModel(feats=1)
        self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=params['lr'], weight_decay=1e-5)
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, 5, 0.9)
        self.criterion = nn.MSELoss()

        self.save_path = None if not params.get('save_path') else params.get('save_path')
    ...
    def test_phase(self, tsData: TSData):
        ...
        with torch.no_grad():
            for idx, (x, _) in loop:
                ...
                elem = x[-1, :, :].view(1, bs, 1)
                _, z = self.model(x)
            loss = self.criterion(z, elem)[0]

        # TODO: TranAD use SPOT